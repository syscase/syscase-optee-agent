...
int parse_argument_vector_64(struct buffer *buffer, struct parse_state *state, sc_u_int64_t *value)
{
  sc_u_int64_t *vector;
  sc_u_int8_t size;
  ...
  if(size && !vector)
    return -1;
  if(syscase_verbose) sc_printf("argument vector 64 %llx - size %d\n", (unsigned long long)(sc_u_long) vector, size);
